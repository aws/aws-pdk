...
class Lambda implements LambdaInterface {
  ...
  public async handler(event: SQSEvent, context: any): Promise<any> {
    ...
    const onConnectHandler = async (message: any): Promise<void> => {
      ...
      try {
        const notifications = await ddb.send(new QueryCommand(getNotificationsCommandInput))
        ...
        for (const notification of notifications.Items!) {
          ...
          logger.info(`sent notification ${notification.notificationId} to ${message.connectionId}`)
        }
      } catch (error: any) {
        logger.error(error);
        throw (error);
