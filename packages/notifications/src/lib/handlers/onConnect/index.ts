...
// const { CONNECTIONS_TABLE_NAME, NOTIFICATIONS_TABLE_NAME, WS_HTTPS_ENDPOINT, WS_API_STAGE } = process.env;
...
const sqs = tracer.captureAWSv3Client(new SQSClient({region: process.env.region}));
...
class Lambda implements LambdaInterface {
  ...
  public async handler(event: APIGatewayProxyEvent, context: any): Promise<any> {
    ...
    try {
      ...
      logger.debug(`Inserting connection details for ${event.requestContext.connectionId}/${authenticatedCustomerId}`);
