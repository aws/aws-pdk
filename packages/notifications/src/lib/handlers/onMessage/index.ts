...
const { NOTIFICATIONS_TABLE_NAME, WS_HTTPS_ENDPOINT, WS_API_STAGE } = process.env;
...
const api = tracer.captureAWSv3Client(new ApiGatewayManagementApiClient({ apiVersion: '2012-08-10', region: process.env.AWS_REGION, endpoint: `${WS_HTTPS_ENDPOINT}/${WS_API_STAGE}` }));
...
class Lambda implements LambdaInterface {
  ...
  public async handler(event: APIGatewayProxyEvent, context: any): Promise<APIGatewayProxyResult> {
    ...
    const acknowledgeNotification = async (props: AcknowledgedNotificationProps): Promise<void> => {
    ...
    const dismissNotification = async (props: DismissNotificationProps): Promise<void> => {
    ...
    try {
      ...
      switch (message.action) {
        case NOTIFICATION_ACTIONS.DISMISS_NOTIFICATION:
          await dismissNotification({ id: message.id, dismissedBy: message.dismissedBy });
          metrics.addMetric('dismissedNotifications', MetricUnits.Count, 1);
          break;
        case NOTIFICATION_ACTIONS.ACKNOWLEDGE_NOTIFICATION:
          await acknowledgeNotification({ id: message.id, acknowledgedBy: message.acknowledgedBy });
